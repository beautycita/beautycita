...
export default function ChatWidget() {
  ...
  const initializeConversation = async () => {
    try {
      ...
      const currentLanguage = i18n.language || 'es-MX'
      const language = currentLanguage.startsWith('es') ? 'es' : 'en'
  ...
  return (
    <>
      ...
      <motion.button
        onClick={toggleChat}
        className="fixed bottom-6 right-6 z-50 w-14 h-14 bg-gradient-rose-gold hover:shadow-rose-gold-glow text-white rounded-3xl shadow-beauty-soft flex items-center justify-center transition-all duration-300"
          ...
          >
            ...
            <div className="bg-gradient-cotton-candy text-white p-4 flex items-center justify-between relative">
              ...
              <div className="flex items-center space-x-3 relative z-10">
                <div className="relative">
                  <div className="w-10 h-10 bg-gradient-rose-gold rounded-3xl flex items-center justify-center shadow-rose-gold-glow">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gradient-to-br from-beauty-blush/10 via-white to-beauty-lavender/10">
                ...
                >
                  ...
                  >
                      ...
                      <div className="mt-3 space-y-2">
                        {message.buttons.map((button, index) => (
                          <motion.button
                            ...
                            className="block w-full text-left px-3 py-2 text-xs bg-gradient-to-r from-beauty-lavender/20 to-beauty-mint/20 hover:from-beauty-lavender/30 hover:to-beauty-mint/30 rounded-3xl border border-beauty-lavender/30 transition-all duration-200 font-medium"
                          >
                            {button.title}
                          </motion.button>
                        ))}
                    ...
                    <p className="text-xs opacity-60 mt-2 font-light">
                      {message.timestamp.toLocaleTimeString(i18n.language || 'es-MX', {
                        hour: '2-digit',
                        minute: '2-digit'
                      })}
                    </p>
              ...
              {isLoading && (
                ...
                >
                  <div className="bg-white/80 backdrop-blur-sm text-gray-800 border border-beauty-blush/20 px-4 py-3 rounded-3xl shadow-beauty-soft">
                    <div className="flex space-x-1">
                      <div className="w-2 h-2 bg-beauty-hot-pink rounded-3xl animate-bounce" />
                      <div className="w-2 h-2 bg-beauty-coral rounded-3xl animate-bounce" style={{ animationDelay: '0.1s' }} />
                      <div className="w-2 h-2 bg-beauty-lavender rounded-3xl animate-bounce" style={{ animationDelay: '0.2s' }} />
                    </div>
                  </div>
                </motion.div>
              )}
            ...
            <div className="p-4 bg-white/90 backdrop-blur-sm border-t border-beauty-blush/20">
              <form onSubmit={handleSubmit} className="flex space-x-3">
                <input
                  ...
                  className="flex-1 px-4 py-3 border border-beauty-blush/30 rounded-3xl focus:outline-none focus:ring-2 focus:ring-beauty-hot-pink focus:border-beauty-hot-pink text-sm bg-white/80 backdrop-blur-sm"
                  disabled={isLoading}
                />
                <motion.button
                  type="submit"
                  ...
                  whileTap={{ scale: 0.95 }}
                  className="px-4 py-3 bg-gradient-rose-gold text-white rounded-3xl hover:shadow-rose-gold-glow focus:outline-none focus:ring-2 focus:ring-beauty-hot-pink disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300"
                >
