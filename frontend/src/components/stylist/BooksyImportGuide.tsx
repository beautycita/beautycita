...
interface BooksyImportGuideProps {
  onComplete: (data: any) => void
  onClose: () => void
}

export default function BooksyImportGuide({ onComplete, onClose }: BooksyImportGuideProps) {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-br from-blue-50 to-cyan-50 rounded-3xl p-6">
        ...
        <ol className="space-y-3">
          ...
          <li className="flex items-start gap-3">
            ...
            <p className="text-gray-700">Log in to your Booksy Business account</p>
          </li>

          <li className="flex items-start gap-3">
            <span className="flex-shrink-0 w-6 h-6 bg-blue-600 text-white rounded-full flex items-center justify-center text-sm font-bold">3</span>
            <p className="text-gray-700">Navigate to your business profile settings</p>
          </li>

          <li className="flex items-start gap-3">
            <span className="flex-shrink-0 w-6 h-6 bg-blue-600 text-white rounded-full flex items-center justify-center text-sm font-bold">4</span>
            <p className="text-gray-700">Copy and paste each field into the form below</p>
          </li>
          ...
          <div key={field.id} className="relative">
            ...
            {field.multiline ? (
              <textarea
                ...
                rows={4}
                className="w-full px-4 py-3 border border-gray-300 rounded-3xl focus:ring-2 focus:ring-blue-500 focus:outline-none transition-colors"
              ...
              <div className="relative">
                ...
                <input
                  ...
                  value={formData[field.id as keyof typeof formData]}
                  onChange={(e) => handleFieldChange(field.id, e.target.value)}
                  className="w-full pl-12 pr-4 py-3 border border-gray-300 rounded-3xl focus:ring-2 focus:ring-blue-500 focus:outline-none transition-colors"
                  placeholder={field.placeholder}
                />
              </div>
            )}
      ...
      <div className="flex gap-3 pt-4">
        ...
        <button
          ...
          className="flex-1 bg-gradient-to-r from-blue-600 to-cyan-500 text-white font-semibold py-3 px-6 rounded-3xl shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-200 flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none"
        >
