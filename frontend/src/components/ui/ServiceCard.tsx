  ...
  return (
    ...
    >
      ...
      <div className="absolute top-4 left-4 flex items-center space-x-2">
        {service.isPopular && (
          <motion.div
            initial={{ scale: 0 }}
            animate={{ scale: 1 }}
            className="bg-gradient-to-r from-beauty-hot-pink to-beauty-electric-purple text-white text-xs px-2 py-1 rounded-3xl font-medium shadow-hot-pink-glow"
          >
            🔥 Popular
          </motion.div>
        )}
        {service.isNew && (
          <motion.div
            ...
            className="bg-gradient-to-r from-beauty-lime-green to-beauty-ocean-blue text-white text-xs px-2 py-1 rounded-3xl font-medium"
          >
            ✨ Nuevo
          </motion.div>
        )}
      ...
      <motion.button
        ...
        whileTap={{ scale: 0.9 }}
        className="absolute top-4 right-4 w-10 h-10 bg-white/90 backdrop-blur-sm rounded-3xl flex items-center justify-center hover:scale-110 transition-transform shadow-beauty-soft z-10"
      >
        {isLiked ? (
      ...
      <div className="relative p-6 h-full flex flex-col justify-between min-h-[280px]">
        ...
        <div className="text-center mb-4">
          <motion.div
            ...
            className="w-16 h-16 mx-auto mb-3 bg-gradient-to-br from-white/50 to-white/20 rounded-3xl flex items-center justify-center text-3xl backdrop-blur-sm border border-white/30"
          >
          ...
          <div className="flex flex-wrap gap-1 justify-center mb-4">
            {service.tags.slice(0, 3).map((tag, index) => (
              <motion.span
                ...
                className="px-2 py-1 bg-beauty-lavender/20 text-beauty-electric-purple text-xs rounded-3xl border border-beauty-lavender/30"
              >
                #{tag}
              </motion.span>
            ))}
