...
export default function SwipeableCard({
  ...
  return (
    <motion.div
      ...
      whileTap={{ scale: 0.95 }}
    >
      <div className="genz-card rounded-2xl overflow-hidden shadow-neon-pink">
        {/* Image Container */}
          ...
          {item.tags && item.tags.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {item.tags.slice(0, 3).map((tag, index) => (
                <span
                  key={index}
                  className="bg-beauty-cyber/20 text-beauty-cyber text-xs px-2 py-1 rounded-full border border-beauty-cyber/30"
                >
                  #{tag}
                </span>
              ))}
