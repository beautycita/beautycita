-- Migration: Pricing ranges, messaging enhancements (final)
-- Created: 2025-10-08
...
-- Add pricing columns to existing video_consultations table
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'service_templates_category_fkey') THEN
    ALTER TABLE service_templates ADD CONSTRAINT service_templates_category_fkey
      FOREIGN KEY (category) REFERENCES service_categories(name) ON DELETE CASCADE;
  END IF;
END$$;
...
-- Insert popular service templates (only if table is empty)
